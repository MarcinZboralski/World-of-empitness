...
public class SECTR_CullingCamera : MonoBehaviour 
{
	...
	{	
		...
		{
			SECTR_Member cullingMember = GetComponent<SECTR_Member>();
	...
	private void _ApplyCulling()
	{
		...
		for(int memberIndex = 0; memberIndex < numMembers; ++memberIndex)
		{
			...
			int numChildren = member.Children.Count;
			for(int childIndex = 0; childIndex < numChildren; ++childIndex)
			{
				SECTR_Member.Child child = member.Children[childIndex];
	...
	private void UndoCulling()
	{
		...
		for(int terrainIndex = 0; terrainIndex < numTerrains; ++terrainIndex)
		{
			...
			// Unity 5.0.1 has a bug that causes this code path to crash.
